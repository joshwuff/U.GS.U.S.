    ...
    function updateCalc() {
        ...
        serviceSelects.forEach(dropdown => {
            ...
            Array.from(dropdown.options).forEach(opt => {
                ...
                // Rule A: Is it already selected elsewhere?
                const isSelectedElsewhere = selectedTags.includes(targetTag) && currentSelection !== targetTag;

                // Rule B: Does it conflict based on the SOP SOW map?
                let isConflicting = false;
                selectedTags.forEach(activeTag => {
                    if (activeTag !== currentSelection) {
                        if (incompatibilities[activeTag] && incompatibilities[activeTag].includes(targetTag)) {
                            isConflicting = true;
                        }
                    }
                });
